...
1. 填写校园网账号、密码，选择运营商（密码框右侧可一键显示 / 隐藏密码）
2. 点击「一 键 登 录」完成认证，顶部状态徽标实时显示连接状态（未连接 / 正在登录 / 已连接 / 失败）
3. 勾选「断线自动重连」并设置检测间隔（默认 300 秒），登录成功后自动监控网络，断线即重连
4. 勾选「开机自启」可实现开机后自动在后台维持连接

> 界面采用卡片式布局并支持 High-DPI 高分屏缩放；「运行控制台」支持一键复制 / 清空日志。  
